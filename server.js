...
function transcode(input, output) {
  return new Promise((resolve, reject) => {
    const args = ['-y', '-i', input, '-t', '175',
      '-vf', "scale=w='if(gte(iw,ih),1920,1080)':h='if(gte(iw,ih),1080,1920)':force_original_aspect_ratio=decrease:force_divisible_by=2",
      '-an', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', output];
...
app.get('/api/scans/:id', async (req, res) => {
  ...
  try {
    ...
    res.json({ state: STATUS[r.data?.status] || 'processing', serialize });
...
app.get('/api/scans/:serialize/model', async (req, res) => {
  ...
  try {
    if (!fs.existsSync(cached)) {
      if (serialize.startsWith('mock-')) {
        await fsp.copyFile(path.join(__dirname, 'public', 'sample-scan.glb'), cached);
      } else {
